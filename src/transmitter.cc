...
void https_transmitter::transmit(std::function<void(std::ostream &input)> function) {
  auto content_buf = std::make_shared<boost::asio::streambuf>();
  {
    std::ostream os(content_buf.get());
    function(os);
  }

  pool.push([content_buf] (std::string certificate_chain_file,
              ...
              boost::asio::connect(socket.lowest_layer(), resolver.resolve(remote_host, std::to_string(port)));
              ...
              boost::asio::streambuf header_buf, from_server;
              socket.async_handshake(
                boost::asio::ssl::stream<boost::asio::ip::tcp::socket>::client,
                [&socket, &header_buf, &content_buf, &from_server] (const boost::system::error_code &error) {
                  if (!error) {
            ...
            }, certificate_chain_file_, remote_host_, port_);
}
